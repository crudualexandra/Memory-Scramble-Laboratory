...
function parseState(state) {
    ...
    if (!m)
        throw new Error("bad header ROWxCOL");
    ...
    return { rows, cols, at, all: lines.slice(1) };
}
async function loadGridFromFile(rel) {
    ...
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
            ...
            const row = grid[r];
            if (!row)
                throw new Error("malformed grid row");
            row[c] = v;
        }
    }
    return grid;
}
function findMatchingPair(grid) {
...
function findNonMatchingPair(grid) {
    const firstRow = grid[0];
    if (grid.length === 0 || !firstRow || firstRow.length === 0)
        throw new Error("empty grid");
    const cols = firstRow.length;
    ...
    if (baseValMaybe === undefined)
        throw new Error("malformed grid cell");
    const baseVal = baseValMaybe;
    // Exclude (0,0) and (0,1) to avoid interfering with test's cleanup/release coordinates
    const excluded = (r, c) => r === 0 && (c === 0 || c === 1);
    let firstPos;
    let firstVal;
    ...
    if (!firstPos || firstVal === undefined)
        throw new Error("no candidates for first non-matching base");
    for (let r = 0; r < grid.length; r++) {
        const row = grid[r];
        if (!row)
            throw new Error("malformed grid row");
...
async function applyMap(b, f, player = "tester") {
    const anyB = b;
    if (typeof anyB.map === "function")
        return anyB.map(f);
    if (typeof anyB.mapCards === "function")
        return anyB.mapCards(player, f);
    throw new Error("Board has no map()/mapCards()");
}
describe("Board map()", () => {
    it("transforms all cards (async) without throwing", async () => {
        const b = await Board.parseFromFile("boards/ab.txt");
        const f = async (card) => card.toUpperCase();
        await applyMap(b, f);
    ...
    it("rejects invalid transformed card", async () => {
        const b = await Board.parseFromFile("boards/perfect.txt");
        const bad = async (_) => "invalid card with spaces";
        await assert.rejects(() => applyMap(b, bad), /invalid transformed card/);
    });
    it("handles concurrent map() calls", async function () {
        this.timeout(5000);
        const b = await Board.parseFromFile("boards/ab.txt");
        const f1 = async (c) => { await new Promise(r => setTimeout(r, 10)); return "X_" + c; };
        const f2 = async (c) => { await new Promise(r => setTimeout(r, 10)); return "Y_" + c; };
