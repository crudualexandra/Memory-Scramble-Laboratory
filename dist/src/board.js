...
export class Board {
    ...
    flipSecond(pos, by) {
        ...
        if (matched) {
            ...
            s1.controller = null;
            s2.controller = null; // 2-E (relinquish immediately)
    ...
    async mapCards(_player, transform) {
        const byValue = new Map();
        for (let r = 0; r < this.rows; r++) {
            for (let c = 0; c < this.cols; c++) {
                const cell = this.getCell(r, c);
        ...
        for (const [orig, positions] of entries) {
            const newVal = mapping.get(orig);
            if (newVal === undefined || newVal === orig)
                continue;
            for (const { r, c } of positions) {
                const cell = this.getCell(r, c);
                if (cell === null)
                    continue;
                if (cell.label !== newVal) {
                    cell.label = newVal;
                    changed = true;
                }
            }
